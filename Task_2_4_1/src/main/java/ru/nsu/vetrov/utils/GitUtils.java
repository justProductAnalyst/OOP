...
public class GitUtils {
    ...
    public static boolean updateRepositories(CheckerConfig config, String repoPrefix) {
        ...
        String token = "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";

        for (Student student : config.getStudents()) {
            try {
                File repoFile = new File(String.format("%s/%s", repoPrefix, student.getNickname()));
                Git repo;
                String repoURI = String.format("https://%s:%s@github.com/%s/OOP", username, token, student.getNickname());
                ...
                if (!repoFile.exists()) {
                    ...
                    try {
                        System.out.println("Pulling latest changes for student: " + student.getNickname());
                        ...
                        System.out.println("Error opening repository for student " + student.getNickname() + ": " + e.getMessage());
                        return true;
                    }
                }
            } catch (GitAPIException | URISyntaxException e) {
                System.out.println("Failed to clone or pull repository for student " + student.getNickname() + ": " + e.getMessage());
            }
        }
        return false;
    }
