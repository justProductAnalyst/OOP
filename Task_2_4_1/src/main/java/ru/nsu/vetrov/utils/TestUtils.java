...
public class TestUtils {
    ...
    public static int getCoveragePercentage(File projectDir,
                                            String repoPrefix, Student student, Task task) {
        runGradleWrapperTask(projectDir, "jacocoTestReport");

        File jacocoFile = new File(
                String.format("%s/%s/%s/build/reports/jacoco/test/jacocoTestReport.xml",
                        repoPrefix, student.getNickname(), task.getName())
        );

        try {
            Document report = Jsoup.parse(jacocoFile, "UTF-8",
                    "", Parser.xmlParser());
    ...
    public static TestCounts getTestCounts(String repoPrefix, Student student, Task task) {
        File reportFile = new File(
                String.format("%s/%s/%s/build/reports/tests/test/index.html",
                        repoPrefix, student.getNickname(), task.getName())
        );
        try {
            Document report = Jsoup.parse(reportFile, "UTF-8");
            int testCount = Integer.parseInt(report.getElementById("tests").getElementsByClass(
                    "counter").get(0).text());
            int failCount = Integer.parseInt(report.getElementById("failures").getElementsByClass(
                    "counter").get(0).text());
            int skipCount = Integer.parseInt(report.getElementById("ignored").getElementsByClass(
                    "counter").get(0).text());
